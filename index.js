    ...
    (async() => {
        for (const fileName of fileNames) {
            ...
            .then((image) => {
                ...
                const expectedNumberCount = config.count;
                const totalDigit = patternWidth + 11 + (patternWidth * expectedNumberCount / 2);
                const unitSize = config.unit;
                
                const totalPixel = unitSize * totalDigit;
                const digits = [];
                for (let x = 0; x < totalPixel; x += unitSize) {
                    const { r, g, b } = Jimp.intToRGBA(image.getPixelColor(config.left + x, config.up));
